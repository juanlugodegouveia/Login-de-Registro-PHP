...
// 	$usuario = stripslashes($usuario);
	...
	if (empty($usuario) or empty($password) or empty($password2)) {
		...
		try {
			$conexion = new PDO('mysql:host=localhost;dbname=loginusuario', 'root', '123456');
		} catch (PDOException $e) {
			echo "Error:" . $e->getMessage();
		}

		$statement = $conexion->prepare('SELECT * FROM usuarios WHERE usuario = :usuario LIMIT 1'); //Limit 1 es por seguridad para que solo traiga un registro
		...
		# pero esto no asegura por completo la informacion encriptada. sha512 es un algoritmo para encryptar
